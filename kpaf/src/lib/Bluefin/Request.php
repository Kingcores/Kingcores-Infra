...
class Request
{
    ...
    {
        $this->_get         = clone $this->_get;
        $this->_post        = clone $this->_post;
        $this->_put         = clone $this->_put;
        $this->_cookie      = clone $this->_cookie;
        $this->_files       = clone $this->_files;
        $this->_server      = clone $this->_server;
        $this->_route       = clone $this->_route;
    }
    ...
    {
        return array_try_get($this->_get, $name, $default, $pop);
    }
     ...
     *
     * @param $withProxyCheck Whether to check the current request is made behind a proxy or not
     *
    ...
    public function getRequestUri()
    {
        ...
        if (!isset($requestUri))
        {
            $iisWasUrlRewritten = $this->getServerParam('IIS_WasUrlRewritten', '0');
            $unencodedUrl = $this->getServerParam('UNENCODED_URL', '');

            if ($iisWasUrlRewritten == '1' && $unencodedUrl != '')
            {
                // IIS7 with URL Rewrite: make sure we get the decoded url (double slash problem)
    ...
    public function getRelativeLandingUrl()
    {
        if (array_key_exists('rel_landing', $this->_cache))
        {
            return $this->_cache['rel_landing'];
        }
        ...
        return ($this->_cache['rel_landing'] = rtrim($baseUrl, '/'));
    }
    ...
    public function getLandingUrl()
    {
        ...
        return $this->getScheme() . '://' . $this->getHttpHost() . $this->getRelativeLandingUrl() . $qs;
    }
    ...
    public function getRelativePath()
    {
        ...
        $baseUrl = $this->getRelativeLandingUrl();
        if (empty($baseUrl))
        {
            return ($this->_cache['rel_path'] = '');
        }
