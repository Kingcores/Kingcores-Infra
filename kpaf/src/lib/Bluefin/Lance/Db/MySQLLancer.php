    ...
    public function translateSQL($command, $target, array $columns, array $data)
    {
        ...
        switch ($command)
        {
            ...
            case 'insert':
                ...
                $modelEntity = $this->_schema->getLoadedModelEntity($target);
            ...
            case 'update':
                ...
                $modelEntity = $schema->getLoadedModelEntity($target);
                if (!isset($modelEntity))
                {
                    throw new GrammarException("Invalid target [{$target}].");
                }
                ...
                for ($offset = 0; $offset < $count; $offset++)
                {
                    ...
                    $pk = $schema->getLoadedModelEntity($target)->entity->getPrimaryKey();
                    $pk = "`{$pk}`";
