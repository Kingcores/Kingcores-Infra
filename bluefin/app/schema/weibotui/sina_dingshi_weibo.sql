...
CREATE TABLE IF NOT EXISTS `sina_dingshi_weibo` (
    `sina_dingshi_weibo_id` INT(10) NOT NULL AUTO_INCREMENT COMMENT 'ID',
    ...
    `errno` INT(10) NOT NULL DEFAULT 0 COMMENT '错误号，非0表示有错误',
    `error` TEXT COMMENT '错误信息',
    `status` VARCHAR(20) NOT NULL DEFAULT 'tosend' COMMENT '发送状态',
    PRIMARY KEY (`sina_dingshi_weibo_id`),
    UNIQUE KEY `uk_sina_dingshi_weibo_k_uid_send_time` (`uid`,`send_time`),
    KEY `ak_sina_dingshi_weibo_k_send_time` (`send_time`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='新浪定时微博' AUTO_INCREMENT=1;