...
class QQWeiboAuth implements AuthInterface
{
    private $_name;
    private $_appName;
    ...
    public function __construct($authName, array $config)
    {
        ...
        {
            throw new \Bluefin\Exception\ConfigException("Invalid qqweibo auth configuration!");
        }
    ...
    {
        $c = QQWeiboClientFactory::createFromConfig('weibotui');
        $o = $c->oauth;

        $url = $o->getAuthorizeURL($this->getResponseUrl($callbackUrl));
    ...
    {
        return isset($redirectUrl) ? build_uri($this->_responseUrl, [Convention::KEYWORD_REQUEST_FROM => $redirectUrl, 'type' => \WBT\Model\Weibotui\WeiboType::QQ_WEIBO], null, false) : $this->_responseUrl;
    }
    ...
    {
        if (!all_keys_exists(['code', 'from','uid'], $authInput))
        {
        ...
        $code = $authInput['code'];
        $uid = $authInput['uid'];
        $redirect_uri = $this->getResponseUrl($authInput['from']);
        ...
        try
        {
            $token = $o->getAccessToken($code, $uid, $redirect_uri);
        }
    ...
    public function setIdentity($identity)
    {
        ...
        $key = "config.qqweibo.{$this->_appName}";
        $weiboConfig = _C($key);
