...
//  通过 $_SESSION['image_verify_code'] 可以获得验证码的值
function process()
{
    ob_clean();
...
class SimpleCaptcha {
    ...
    var $wordsFile = null;//'words/en.php';
    ...
    var $session_var = 'image_verify_code';//'captcha';
    ...
    function CreateImage() {
        ...
        App::getInstance()->session()->set($this->session_var, $text);
    ...
    function GetRandomCaptchaText() {
        ...
        $text  = "";
        for ($i=0; $i<$length; $i++) {
            $text .= substr($words, mt_rand(0, $words_len -1), 1);
        }
        //log_debug("text=$text");
        return $text;
    }
    ...
    function WriteText($text, $fontcfg = array()) {
        ...
        $lettersMissing = 0;//$this->maxWordLength-strlen($text);
